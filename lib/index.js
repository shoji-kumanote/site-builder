...
var terminalSize = require('term-size');
var url = require('url');
...
var terminalSize__default = /*#__PURE__*/_interopDefaultLegacy(terminalSize);
...
class Transit {
    ...
    static async create(entryPath, srcFilePath, distFilePath) {
        const transit = new Transit(await fs__default["default"].promises.readFile(srcFilePath, "utf-8"), undefined, entryPath, srcFilePath, path__default["default"].basename(srcFilePath), distFilePath, distFilePath === undefined ? undefined : path__default["default"].basename(distFilePath));
        return transit;
    }
    /**
        ...
        if (workFlow.binary) {
            ...
            if (workFlow.distPath !== undefined) {
                ...
                if (workFlow.sourceMap && result.sourceMap) {
                    await context.file.write(path__default["default"].resolve(context.config.dist, workFlow.distPath), annotateSourceMap(result.data, `${path__default["default"].basename(workFlow.distPath)}.map`));
                    await context.file.write(path__default["default"].resolve(context.config.dist, `${workFlow.distPath}.map`), result.sourceMap);
                }
                else {
                    await context.file.write(path__default["default"].resolve(context.config.dist, workFlow.distPath), annotateSourceMap(result.data));
                }
                context.logger.end();
                context.logger.info();
            }
...
const buildEntry = async (context, entry) => {
    ...
    const transit = await Transit.create(entry.entryPath, entry.srcFilePath, distPath === undefined
        ? undefined
        : path__default["default"].resolve(context.config.dist, distPath));
...
const clean = async (context) => {
    ...
    const files = entries
        ...
        .map((x) => path__default["default"].resolve(config.dist, x));
...
const getEntryType = (vendor, filePath) => {
    ...
    const lib = /^_/.test(path__default["default"].basename(filePath));
    if (micromatch__default["default"]([filePath], vendor).length === 1) {
...
const unBuildFile = async (context, filePath) => {
    ...
    if (distFilePaths.length > 0) {
        for (const distFilePath of distFilePaths) {
            // eslint-disable-next-line no-await-in-loop
            await context.file.unlink(path__default["default"].resolve(context.config.dist, distFilePath));
        }
    }
};
...
const watch = async (context) => {
    ...
    const chokidar = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('chokidar')); });
    const watcher = chokidar.watch([
        ...context.config.src.map((x) => path__default["default"].resolve(x, "**", "*")),
...
const load = async (filePath) => {
    try {
        return jsYaml__default["default"].load(await fs__default["default"].promises.readFile(filePath, "utf8"));
    }
    catch {
...
class Config {
    ...
    static async create(cliOption) {
        ...
        const config = cliOption.config.map((x) => path__default["default"].resolve(x));
    ...
    constructor(data, pageData) {
        ...
        this.dev = data.dev;
        this.base = path__default["default"].resolve(data.base);
        this.dist = path__default["default"].resolve(data.dist);
        this.#src = data.src.map((x) => path__default["default"].resolve(x));
        this.#vendor = data.vendor.map((x) => path__default["default"].resolve(x));
        this.#ignore = data.ignore.map((x) => path__default["default"].resolve(x));
...
class Entry {
    ...
    constructor(config, filePath) {
        ...
        this.entryPath = path__default["default"].relative(config.base, filePath);
    }
    /**
     * フィルタが有効かどうか
     *
...
class File {
    ...
    async removeDirs(dirPath, first) {
        try {
            if (!(await fs__default["default"].promises.stat(dirPath)).isDirectory())
                return;
        }
        catch (e) {
        ...
        for (const entry of await fs__default["default"].promises.readdir(dirPath)) {
            // eslint-disable-next-line no-await-in-loop
            await this.removeDirs(path__default["default"].resolve(dirPath, entry), false);
        }
        if (first)
            return;
        const entries = await fs__default["default"].promises.readdir(dirPath);
        if (entries.length === 0) {
            try {
                if (!this.dryRun)
                    await fs__default["default"].promises.rmdir(dirPath);
    ...
    async prepareWrite(filePath, withUnlink = false) {
        if (!this.dryRun) {
            await fs__default["default"].promises.mkdir(path__default["default"].dirname(filePath), { recursive: true });
            if (withUnlink) {
                try {
                    await fs__default["default"].promises.unlink(filePath);
                }
    ...
    async copy(srcFilePath, distFilePath) {
        ...
        if (!this.dryRun) {
            try {
                await fs__default["default"].promises.copyFile(srcFilePath, distFilePath);
            }
            catch (e) {
...
/** コンソールの幅取得 */
const width = () => terminalSize__default["default"]().columns;
/**
 * 色指定を反映
 *
 * @param value - 文字列
...
const applyColor = (value, ...colors) => {
    ...
    for (const color of colors) {
        if (typeof color === "string") {
            result = chalk__default["default"][color](result);
        }
        else {
...
class Logger {
    ...
    idle(timeoutMs = 150) {
        ...
        this.#idleTimer = setTimeout(() => {
            this.banner(dateFns.format(new Date(), "HH:mm:ss"), "gray");
            console.info("");
        }, timeoutMs);
    }
    /**
    ...
    getFileInfo(filePath) {
        for (const { name, dirPath } of this.dirs) {
            const relPath = path__default["default"].relative(dirPath, filePath);
            if (!relPath.includes(".."))
                return { name, relPath };
        }
...
class Context {
    ...
    /** 全ソースファイルのエントリ取得 */
    async getEntries() {
        const filePaths = await globby.globby([...this.config.src, ...this.config.ignore.map((x) => `!${x}`)], { onlyFiles: true });
        return filePaths.map((x) => new Entry(this.#config, x));
    }
    /** 指定ファイルのエントリ取得 */
    getEntry(filePath) {
        if (micromatch__default["default"]([filePath], this.config.ignore).length > 0) {
            return undefined;
        }
...
const findFile = (baseDir, file) => {
    const filePath = path__default["default"].resolve(baseDir, file);
    ...
    if (!/^_/.test(baseName)) {
        targets.push(path__default["default"].resolve(dirName, `_${baseName}`));
        if (!/\.hbs$/.test(baseName)) {
            targets.push(path__default["default"].resolve(dirName, `_${baseName}.hbs`));
        }
        if (!/\.tpl$/.test(baseName)) {
            targets.push(path__default["default"].resolve(dirName, `_${baseName}.tpl`));
        }
        if (!/\.html$/.test(baseName)) {
            targets.push(path__default["default"].resolve(dirName, `_${baseName}.html`));
        }
    }
...
const create = async (transit, context) => {
    const handlebars = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('handlebars')); });
    ...
    const apply = (file, currentData, hbsContext, include) => {
        ...
        const includeFilePath = findFile(path__default["default"].dirname(baseFilePath), file);
        context.dependency.add(transit.srcFilePath, includeFilePath);
        const src = fs__default["default"].readFileSync(includeFilePath, "utf-8").trim();
        if (!include)
            return src;
...
const hbsTransform = async (transit, context) => {
    try {
        ...
        const result = compiled({
            ...
            __PATH__: path__default["default"].relative(context.config.dist, transit.distFilePath ?? ""),
            __ROOT__: path__default["default"].relative(transit.distFilePath ?? "", context.config.dist),
            __TIMESTAMP__: stamp,
...
const htmlFormat = async (transit, context) => {
    if (context.config.dev)
        return transit;
    const prettier = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('prettier')); });
...
const cssOptimize = async (transit, context) => {
    ...
    const autoprefixer = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('autoprefixer')); });
    const mqpacker = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('css-mqpacker')); });
    const sorting = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('postcss-sorting')); });
...
const cssMinify = async (transit, context) => {
    if (context.config.dev)
        return transit;
    const postcss = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('postcss')); });
    const csso = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('postcss-csso')); });
    const { css, map } = postcss
    ...
    mapJSON.sources = mapJSON.sources.map((x) => {
        if (/^file/.test(x)) {
            return path__default["default"].relative(path__default["default"].dirname(transit.srcFilePath), new url__default["default"].URL(x).pathname);
        }
        return x;
    });
...
const sassCompile = async (transit, context) => {
    const sass = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('sass')); });
    const result = await sass.default.compileStringAsync(transit.data, {
        syntax: /sass$/.test(transit.srcFileName) ? "indented" : "scss",
        loadPaths: [path__default["default"].dirname(transit.srcFilePath)],
        sourceMap: true,
...
const transpile = async (transit) => {
    const babel = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('@babel/core')); });
    const env = (await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('@babel/preset-env')); })).default;
...
const jsMinify = async (transit, context) => {
    if (context.config.dev)
        return transit;
    const terser = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('terser')); });
...
const getEsBuildResult = (esBuildResult, srcDirPath) => {
    const result = [
        "",
        undefined,
        Object.keys(esBuildResult.metafile?.inputs ?? {}).map((x) => path__default["default"].resolve(srcDirPath, path__default["default"].relative(srcDirPath, x))),
    ];
...
const mjsBundle = async (transit, context) => {
    const esbuild = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('esbuild')); });
...
const jpegOptimize = async (transit, context) => {
    ...
    const jpegTran = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('jpegtran-bin')); });
    ...
    await util.promisify(child_process.execFile)(jpegTran.default, [
        "-progressive",
        "-optimize",
        "-outfile",
...
const pngOptimize = async (transit, context) => {
    ...
    const optipng = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('optipng-bin')); });
    if (transit.distFilePath === undefined) {
        throw new Error(`distFilePath not set`);
    }
    await context.file.prepareWrite(transit.distFilePath, true);
    await util.promisify(child_process.execFile)(optipng.default, [
        "-clobber",
        "-i",
        "1",
        "-out",
...
const gifOptimize = async (transit, context) => {
    ...
    const gifsicle = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('gifsicle')); });
    if (transit.distFilePath === undefined) {
        throw new Error(`distFilePath not set`);
    }
    await context.file.prepareWrite(transit.distFilePath, true);
    await util.promisify(child_process.execFile)(gifsicle.default, [
        "--interlace",
        "--output",
        transit.distFilePath,
        transit.srcFilePath,
...
const svgOptimize = async (transit, context) => {
    if (context.config.dev)
        return transit;
    const svgo = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('svgo')); });
