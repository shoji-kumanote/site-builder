    ...
    getWorkFlows() {
        ...
        switch (this.entryType) {
            ...
            case ENTRY_TYPES.php:
                return getHbsWorkFlows(this.entryPath.replace(/\.php.hbs$/, ".php"), this.isFilterEnabled(FILTER_TYPES.hbsTransform), false);
...
class Logger {
     ...
     */
    entry(entry, ...colors) {
        this.info(applyColor("*", "dim"), applyColor(`${entry.entryPath} [${entry.entryType}]`, colors.length === 0 ? "green" : colors));
    }
    /**
     * フィルタ種別についてログ出力
     *
...
class Context {
    ...
    async getEntries() {
        ...
        const filePaths = await globby(path === path.win32
            ? patterns.map((x) => x.split(path.sep).join("/"))
            : patterns, { onlyFiles: true });
        return (path.win32 === path
            ? filePaths.map((x) => x.split("/").join(path.sep))
            : filePaths).map((x) => new Entry(this.#config, x));
    }
    /** 指定ファイルのエントリ取得 */
    getEntry(filePath) {
 ...
 * @throws ファイルがない場合
 */
const getFilePath = (baseDir, includeTargetDir, file) => {
 ...
 * @throws ファイルがない場合
 */
const findFile = (baseDir, includeTargetDir, file) => {
...
const create = async (transit, context) => {
    ...
    const imageSrcSize = (file, hbsContext, srcAttr) => {
        // eslint-disable-next-line no-underscore-dangle
        const baseFilePath = hbsContext.data.root.__FILE__;
        const filePath = getFilePath(context.config.base, path.dirname(baseFilePath), file);
    ...
    const apply = (file, currentData, hbsContext, include) => {
        // eslint-disable-next-line no-underscore-dangle
        const baseFilePath = hbsContext.data.root.__FILE__;
        const includeFilePath = findFile(context.config.base, path.dirname(baseFilePath), file);
    ...
    hbs.registerHelper("_", function includeHelper(file, hbsContext) {
        return new handlebars.default.SafeString(apply(hbs.compile(file)(this), this, hbsContext, true));
    });
    ...
    hbs.registerHelper("$", function insertHelper(file, hbsContext) {
        return new handlebars.default.SafeString(apply(hbs.compile(file)(this), this, hbsContext, false));
    });
    hbs.registerHelper("insert", function insertHelper(file, hbsContext) {
        return new handlebars.default.SafeString(apply(hbs.compile(file)(this), this, hbsContext, false));
    });
    hbs.registerHelper("image", function imgHelper(file, arg1, arg2) {
        const attrName = typeof arg2 !== "undefined" ? arg1 : "src";
        const hbsContext = arg2 ?? arg1;
        return new handlebars.default.SafeString(imageSrcSize(hbs.compile(file)(this), hbsContext, attrName));
    });
    return hbs;
};
/** hbs transform フィルタ */
...
const sassCompile = async (transit, context) => {
    ...
    context.dependency.add(transit.srcFilePath, ...result.loadedUrls.map((x) => path.win32 === path
