...
var init_sassCompile = __esm({
  "src/filters/sassCompile.ts"() {
    sassCompile = async (transit, context) => {
      ...
      context.dependency.add(transit.srcFilePath, ...result.loadedUrls.map((x) => path4.relative(context.config.base, x.pathname)));
...
var clean = async (context) => {
  ...
  const entries = await context.getEntries();
  const files = entries.reduce((prev, x) => [...prev, ...x.getDistPaths()], []).map((x) => path8.resolve(config.dist, x));
...
var Config = class {
  ...
  static async create(cliOption) {
    ...
    const config = cliOption.config.map((x) => path9.resolve(x));
  ...
  constructor(data, pageData) {
    ...
    this.dev = data.dev;
    this.base = path9.resolve(data.base);
    this.dist = path9.resolve(data.dist);
    this.#src = data.src.map((x) => path9.resolve(x));
    this.#vendor = data.vendor.map((x) => path9.resolve(x));
    this.#ignore = data.ignore.map((x) => path9.resolve(x));
...
var getCssWorkFlows = (entryPath, optimize, minify, format) => {
  if (optimize) {
    if (minify) {
      if (format) {
        return [
...
var getJsWorkFlows = (entryPath, optimize, minify, format) => {
  if (optimize) {
    if (minify) {
      if (format) {
        return [
...
var getEntryType = (vendor, filePath) => {
  const extName = path10.extname(filePath);
  const lib = /^_/.test(path10.basename(filePath));
...
var Entry = class {
  ...
  constructor(config, filePath) {
    ...
    this.entryPath = path11.relative(config.base, filePath);
  }
...
var File = class {
  ...
  async removeDirs(dirPath, first) {
    ...
    for (const entry of await fs4.promises.readdir(dirPath)) {
      await this.removeDirs(path12.resolve(dirPath, entry), false);
    }
    if (first)
      return;
  ...
  async prepareWrite(filePath, withUnlink = false) {
    if (!this.dryRun) {
      await fs4.promises.mkdir(path12.dirname(filePath), { recursive: true });
      if (withUnlink) {
        try {
          await fs4.promises.unlink(filePath);
        } catch {
...
var Logger = class {
  ...
  getFileInfo(filePath) {
    for (const { name, dirPath } of this.dirs) {
      const relPath = path13.relative(dirPath, filePath);
      if (!relPath.includes(".."))
        return { name, relPath };
    }
  ...
  file(label, color, filePath, error = void 0) {
    ...
    this.info(`[${applyColor(label, color)}] ${applyColor(`${name}: `, "dim")}${relPath}${message}`);
  }
